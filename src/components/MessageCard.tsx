...
export const MessageCard = ({data, onPress}: MessageCardProps) => {
  ...
  const chatName =
    otherUsers?.length <= 1 && !data?.isProgramChat
    ...
    <>
      <TouchableOpacity style={styles.container} onPress={onPress}>
        ...
        {data?.members?.length < 3 && data?.program_id == null && (
          <Avatar.Text
            label={
              data?.members[1]?.firstname[0] + data?.members[1]?.lastname[0]
            }
            size={40}
            theme={{
              colors: {
        ...
        {data?.program_id != null && (
          <Avatar.Image source={{uri: data?.pfp}} size={40} />
        )}

        <View style={styles.textContainer}>
          <Text style={styles.name}>
            {data?.program_id ? data.name : chatName}
          </Text>
          {data?.program_id != null && (
